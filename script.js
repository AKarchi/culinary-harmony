...
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFzbG5qcGh1YXdtaHNnaW11ZGVtIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODg4MjYxMzgsImV4cCI6MjEwNDQwMjEzOH0.O3Z6pasa2yYXJc72Vhn5jYVNuyMoWQ2fslMSfVBGeFA';
const supabaseClient = window.supabase.createClient(supabaseUrl, supabaseKey);

let currentCategory='',selectedDish=null,currentPortion=2,basePortion=2;
let favorites=JSON.parse(localStorage.getItem('favorites'))||[];
let recentlyViewed=JSON.parse(localStorage.getItem('recentlyViewed'))||[];
let recipeNotes=JSON.parse(localStorage.getItem('recipeNotes'))||{};
let allRecipes=[],currentRecipes=[];
let plateSelected=false,isOnFavoritesPage=false;
let cookingStepIndex=0,cookingSteps=[];
let timerInterval = null;
let timerSeconds = 0;
let timerRunning = false;
let timerTargetTime = null;
let checkedIngredients=new Set(),completedSteps=new Set();
...
const RECIPE_CATEGORIES = ['breakfast','brunch','lunch','dinner','snacks','desserts','drinks'];

const lanternQuotes = [
  { quote: "You're the ADOBO to my everything — savory, steady, and impossible to forget."},
  ...
  { quote: "You had me at TARAON — because every meal tastes better when it's with you."}
];

document.addEventListener('DOMContentLoaded',()=>{
  initSplash();initPlates();initNavbar();createSpiceParticles();
  loadRecipes();
  initLogoAnimation();
...
async function loadRecipes() {
  ...
  try {
    ...
    allCategoryData.forEach((categoryRecipes, index) => {
      ...
      let normalizedCategory = categoryType.charAt(0).toUpperCase() + categoryType.slice(1);
...
function generateFallbackRecipes() {
  allRecipes = [
    {idMeal:'jp1',strMeal:"Japanese Breakfast Set",strMealThumb:"https://images.unsplash.com/photo-1553621042-f6e147245475?w=500",strArea:"Japanese",strCategory:"Breakfast",difficulty:'medium',prepTime:20,cookTime:15,mood:'healthy',strInstructions:"Grill salmon until cooked.\nCook rice in rice cooker.\nPrepare miso soup.\nArrange with pickles and nori.\nServe immediately.",strIngredient1:"Salmon",strMeasure1:"200g",strIngredient2:"Rice",strMeasure2:"2 cups",strIngredient3:"Miso paste",strMeasure3:"2 tbsp",strIngredient4:"Nori",strMeasure4:"2 sheets",strIngredient5:"Pickles",strMeasure5:"100g"},
    {idMeal:'jp2',strMeal:"Chicken Teriyaki",strMealThumb:"https://images.unsplash.com/photo-1569058242567-93de6f36f8e6?w=500",strArea:"Japanese",strCategory:"Lunch",difficulty:'easy',prepTime:10,cookTime:15,mood:'quick',strInstructions:"Mix soy sauce, mirin, and sugar.\nCook chicken until browned.\nPour sauce over chicken and simmer until thick.\nServe with steamed rice.",strIngredient1:"Chicken thighs",strMeasure1:"500g",strIngredient2:"Soy sauce",strMeasure2:"3 tbsp",strIngredient3:"Mirin",strMeasure3:"2 tbsp",strIngredient4:"Sugar",strMeasure4:"1 tbsp",strIngredient5:"Ginger",strMeasure5:"1 tsp"},
    {idMeal:'jp3',strMeal:"Matcha Latte",strMealThumb:"https://images.unsplash.com/photo-1515823662972-da6a2e4d3114?w=500",strArea:"Japanese",strCategory:"Drinks",difficulty:'easy',prepTime:5,cookTime:5,mood:'quick',strInstructions:"Sift matcha powder.\nAdd hot water and whisk.\nSteam milk until frothy.\nPour milk over matcha.",strIngredient1:"Matcha powder",strMeasure1:"2 tsp",strIngredient2:"Hot water",strMeasure2:"1/4 cup",strIngredient3:"Milk",strMeasure3:"1 cup",strIngredient4:"Honey",strMeasure4:"1 tbsp"},
    {idMeal:'jp4',strMeal:"Mochi Ice Cream",strMealThumb:"https://images.unsplash.com/photo-1563805042-7684c019e1cb?w=500",strArea:"Japanese",strCategory:"Dessert",difficulty:'hard',prepTime:30,cookTime:10,mood:'impressive',strInstructions:"Mix glutinous rice flour and sugar.\nAdd water and microwave.\nWrap ice cream balls in mochi.\nFreeze for 1 hour.",strIngredient1:"Glutinous rice flour",strMeasure1:"1 cup",strIngredient2:"Sugar",strMeasure2:"1/4 cup",strIngredient3:"Water",strMeasure3:"1 cup",strIngredient4:"Ice cream",strMeasure4:"1 pint",strIngredient5:"Cornstarch",strMeasure5:"for dusting"},
    {idMeal:'th1',strMeal:"Pad Thai",strMealThumb:"https://images.unsplash.com/photo-1559314809-0d155014e29e?w=500",strArea:"Thai",strCategory:"Lunch",difficulty:'medium',prepTime:20,cookTime:15,mood:'impressive',strInstructions:"Soak rice noodles.\nStir-fry shrimp and tofu.\nAdd noodles and sauce.\nToss with peanuts and lime.",strIngredient1:"Rice noodles",strMeasure1:"200g",strIngredient2:"Shrimp",strMeasure2:"200g",strIngredient3:"Tofu",strMeasure3:"100g",strIngredient4:"Tamarind paste",strMeasure4:"2 tbsp",strIngredient5:"Peanuts",strMeasure5:"1/4 cup"},
    {idMeal:'th2',strMeal:"Mango Sticky Rice",strMealThumb:"https://images.unsplash.com/photo-1596040033229-a9821ebd058d?w=500",strArea:"Thai",strCategory:"Dessert",difficulty:'easy',prepTime:10,cookTime:30,mood:'comfort',strInstructions:"Cook sticky rice in coconut milk.\nLet stand 30 minutes.\nSlice ripe mango.\nServe rice with mango.",strIngredient1:"Sticky rice",strMeasure1:"1 cup",strIngredient2:"Coconut milk",strMeasure2:"1 cup",strIngredient3:"Sugar",strMeasure3:"2 tbsp",strIngredient4:"Mango",strMeasure4:"2 ripe",strIngredient5:"Salt",strMeasure5:"1/4 tsp"},
    {idMeal:'ph1',strMeal:"Chicken Adobo",strMealThumb:"https://images.unsplash.com/photo-1604329760661-e71dc83f8f26?w=500",strArea:"Filipino",strCategory:"Dinner",difficulty:'easy',prepTime:10,cookTime:40,mood:'comfort',strInstructions:"Marinate chicken in soy sauce, vinegar, garlic.\nSauté garlic.\nAdd chicken and brown.\nSimmer 40 mins.",strIngredient1:"Chicken",strMeasure1:"1kg",strIngredient2:"Soy sauce",strMeasure2:"1/2 cup",strIngredient3:"Vinegar",strMeasure3:"1/2 cup",strIngredient4:"Garlic",strMeasure4:"6 cloves",strIngredient5:"Bay leaves",strMeasure5:"3"},
    {idMeal:'kr1',strMeal:"Bibimbap",strMealThumb:"https://images.unsplash.com/photo-1580651315530-69c8e10225cb?w=500",strArea:"Korean",strCategory:"Lunch",difficulty:'medium',prepTime:30,cookTime:20,mood:'healthy',strInstructions:"Cook rice.\nSauté vegetables.\nFry egg.\nArrange over rice with gochujang.",strIngredient1:"Rice",strMeasure1:"2 cups",strIngredient2:"Spinach",strMeasure2:"1 cup",strIngredient3:"Carrots",strMeasure3:"2",strIngredient4:"Bean sprouts",strMeasure4:"1 cup",strIngredient5:"Gochujang",strMeasure5:"2 tbsp"}
  ];
  currentRecipes = [...allRecipes];
}
...
function getDifficulty(r){
  ...
  const ingredients=Object.keys(r).filter(k=>k.startsWith('strIngredient')&&r[k]).length;
  if(steps<=5&&ingredients<=6)return'easy';
  if(steps<=10&&ingredients<=12)return'medium';
  return'hard';
}
function getPrepTime(r){return Math.floor(Math.random()*15)+5;}
function getCookTime(r){return Math.floor(Math.random()*40)+15;}
function getMood(r){
...
function filterRecipesByCategory(category){
  const categoryMap={'breakfast':'Breakfast','brunch':'Brunch','lunch':'Lunch','merienda':'Snack','dinner':'Dinner','dessert':'Dessert','drinks':'Drinks'};
...
function renderFeatured(){
  ...
  const cuisine=cuisines.find(c=>c.label===featured.strArea)||cuisines[0];
...
function renderRecentlyViewed(){
  ...
  grid.innerHTML=recentlyViewed.map(r=>`<div class="recently-card" onclick='showRecipeById("${r.idMeal}")'><div class="recently-img" style="background-image:url('${r.strMealThumb}')"></div><div class="recently-name">${r.strMeal}</div></div>`).join('');
}
...
function showRecipe(recipe){
  ...
  checkedIngredients=new Set();completedSteps=new Set();cookingDoneSteps=new Set();
  ...
  testImg.onerror = () => {
    ...
    recipeImageEl.style.backgroundImage = 'linear-gradient(135deg, var(--beige) 0%, var(--rice-paper) 100%)';
  };
  testImg.src = imagePath;

  const cuisine = cuisines.find(c => c.label === recipe.strArea) || cuisines[0];
  document.getElementById('recipeCuisine').innerHTML = `<span class="flag">${cuisine.flag}</span><span>${recipe.strArea || 'International'}</span>`;
  ...
  document.getElementById('instructionList').innerHTML = instructions.map((inst, idx) => 
  ...
  document.getElementById('chefTip').textContent = recipe.chefTip || chefTips[Math.floor(Math.random() * chefTips.length)];
...
function renderIngredients(ingredients){
  document.getElementById('ingredientList').innerHTML=ingredients.map((ing,idx)=>`<li class="ingredient-item" onclick="toggleIngredient(${idx},this)"><div class="ingredient-check"></div><span class="ingredient-qty">${ing.measure}</span><span>${ing.name}</span></li>`).join('');
}
...
function renderRelatedRecipes(current){
  const related=allRecipes.filter(r=>r.idMeal!==current.idMeal&&(r.strArea===current.strArea||r.strCategory===current.strCategory)).slice(0,3);
  const grid=document.getElementById('relatedGrid');
  if(related.length===0){document.getElementById('relatedSection').style.display='none';return;}
  document.getElementById('relatedSection').style.display='block';
  grid.innerHTML=related.map(r=>`<div class="related-card" onclick='showRecipeById("${r.idMeal}")'><div class="related-img" style="background-image:url('${r.strMealThumb}')"></div><div class="related-name">${r.strMeal}</div></div>`).join('');
}

function closeRecipe(){
  const modal=document.getElementById('recipeModal');
  const book=document.getElementById('recipeBook');
  anime({targets:'.recipe-book',rotateY:[0,20],translateX:[0,50],opacity:[1,0],duration:600,easing:'easeInQuad',
    complete:()=>{modal.style.display='none';book.classList.remove('open');}
  });
}

function toggleIngredient(idx,el){
  if(checkedIngredients.has(idx)){checkedIngredients.delete(idx);el.classList.remove('checked');}
  else{checkedIngredients.add(idx);el.classList.add('checked');}
}

function toggleStep(idx,el){
  if(completedSteps.has(idx)){completedSteps.delete(idx);el.classList.remove('completed');}
  else{completedSteps.add(idx);el.classList.add('completed');}
  updateProgress();
...
function adjustPortion(delta){
  ...
  anime({targets:items,translateX:[-10,0],opacity:[0.5,1],duration:400,delay:anime.stagger(30),easing:'easeOutQuad'});
  renderIngredients(scaledIngredients);
  showToast(`Adjusted to ${currentPortion} serving${currentPortion>1?'s':''}`,'success');
}
...
function toggleStar(){
  ...
  else{favorites.push(selectedDish);showToast('⭐ Added to favorites!','success');launchConfetti();}
  localStorage.setItem('favorites',JSON.stringify(favorites));
  updateStarButton();
  if(isOnFavoritesPage){currentRecipes=[...favorites];renderDishes();}
}
...
function toggleCookingMode(){
  ...
  if(selectedDish&&selectedDish.strMealThumb){
    document.getElementById('cookingRecipeThumb').style.backgroundImage=`url(${selectedDish.strMealThumb})`;
  }
  cookingStepIndex=0;
  cookingDoneSteps=new Set();
  renderCookingStep();
  anime({targets:'#cookingMode',opacity:[0,1],duration:500,easing:'easeOutQuad'});
  anime({targets:'.cooking-step-display',translateY:[30,0],opacity:[0,1],duration:600,delay:200,easing:'easeOutQuad'});
}

function exitCookingMode(){
  anime({targets:'#cookingMode',opacity:[1,0],duration:300,easing:'easeInQuad',complete:()=>{document.getElementById('cookingMode').classList.remove('active');}});
}
...
function toggleMarkDone(){
  ...
  if(cookingDoneSteps.has(cookingStepIndex)){
    ...
    anime({targets:btn,scale:[1,1.1,1],duration:400,easing:'easeOutElastic(1, .6)'});
  }
  updateCookingDots();
...
function renderCookingStep(){
  ...
  document.getElementById('cookingProgressTopFill').style.width=progressPct+'%';
  ...
  if(cookingStepIndex>=total){
    document.getElementById('cookingStepDisplay').innerHTML=`<div class="cooking-complete"><div class="cooking-complete-icon"></div><div class="cooking-complete-title">All Done!</div><div class="cooking-complete-text">Your dish is ready! Enjoy your meal.</div><div class="cooking-controls"><button class="cooking-btn primary" onclick="exitCookingMode()">✕ Exit Cooking Mode</button></div></div>`;
    launchConfetti();
    return;
  }
  ...
  anime({targets:stepText,opacity:[0,1],translateY:[10,0],duration:400,easing:'easeOutQuad'});
  ...
  if(nextBtn){
    nextBtn.textContent=cookingStepIndex===total-1?'Finish ✓':'Next Step →';
  }
...
function jumpToStep(index){
  ...
  renderCookingStep();
  anime({targets:'.cooking-step-text',opacity:[0,1],translateX:[20,0],duration:400,easing:'easeOutQuad'});
}
...
function nextCookingStep(){
  if(cookingStepIndex<cookingSteps.length){
    cookingStepIndex++;
    renderCookingStep();
    anime({targets:'.cooking-step-display',translateX:[30,0],opacity:[0.7,1],duration:400,easing:'easeOutQuad'});
  }
}
...
function printRecipe(){window.print();showToast('Print dialog opened','success');}
async function shareRecipe(){
  if(navigator.share&&selectedDish){
    try{await navigator.share({title:selectedDish.strMeal,text:`Check out this recipe: ${selectedDish.strMeal}`,url:window.location.href});}
    catch(e){}
  }else{
    navigator.clipboard.writeText(`${selectedDish.strMeal} - ${window.location.href}`);
    showToast('Link copied to clipboard!','success');
  }
}

function showFavorites(){isOnFavoritesPage=true;showBrowsePage(true);}
...
function showBrowsePage(isFavorites=false,customRecipes=null){
  ...
  if(isFavorites){
    ...
    searchBar.style.display='none';filterBar.style.display='none';
    ...
    searchBar.style.display='block';filterBar.style.display='flex';
    initCuisineFilters();
    currentRecipes=customRecipes||[...allRecipes];
    setActiveNavButton('browse');
  }
  renderDishes();
  anime({targets:'#browse-page',opacity:[0,1],translateY:[30,0],duration:800,easing:'easeOutQuad'});
}
...
function showMainPage(){
  ...
  document.getElementById('quickSpinContainer').style.opacity='0';
  document.querySelectorAll('.mood-btn').forEach(b=>b.classList.remove('active'));
  document.querySelectorAll('.plate').forEach(p=>{p.classList.remove('active','orbiting','spinning');});
  currentMealType = 'all';
  currentCuisine = 'all';
  ...
  anime({targets:'#main-container',opacity:[0,1],duration:800,easing:'easeOutQuad'});
  anime({targets:'.page-title',opacity:[0,1],translateY:[20,0],duration:800,delay:200});
  renderRecentlyViewed();
}

function initCuisineFilters(){
  const container=document.getElementById('cuisineFilter');
  container.innerHTML=cuisines.map(c=>`<button class="filter-btn ${c.id==='all'?'active':''}" onclick="filterByCuisine('${c.id}',this)"><span>${c.flag}</span> ${c.label}</button>`).join('');
}

function filterByCuisine(cuisine, btn){
  document.querySelectorAll('.cuisine-filter .filter-btn').forEach(b=>b.classList.remove('active'));
  btn.classList.add('active');
  ...
  if (cuisine === 'all') {
    ...
    if (!mealFilterVisible) {
      ...
      anime({targets:'.meal-btn',opacity:[0,1],translateY:[20,0],scale:[0.9,1],delay:anime.stagger(80,{start:100}),duration:600,easing:'easeOutBack'});
    }
  }
...
function filterByMealType(mealType, btn) {
  ...
  document.querySelectorAll('.meal-type-filter .filter-btn').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
...
function applyMealTypeFilter() {
  ...
  let filtered = currentCuisine === 'all' 
    ? [...sourceList] 
    : sourceList.filter(r => {
        const recipeArea = (r.strArea || '').toLowerCase();
        const filterArea = currentCuisine.toLowerCase();
...
function searchDishes(){
  ...
  currentRecipes=!query?[...allRecipes]:allRecipes.filter(r=>r.strMeal.toLowerCase().includes(query));
  renderDishes();
}
...
function initNavbar(){
  anime({targets:'#navbar',translateY:[-100,0],duration:1200,easing:'easeOutQuad',delay:2500});
  anime({targets:'.page-title',opacity:[0,1],translateY:[20,0],duration:800,delay:3000});
}
...
function handleKeyboard(e){
  if(document.getElementById('cookingMode').classList.contains('active')){
    if(e.key==='ArrowRight'||e.key===' '){e.preventDefault();nextCookingStep();}
    else if(e.key==='ArrowLeft'){e.preventDefault();prevCookingStep();}
    else if(e.key==='d'||e.key==='D'){e.preventDefault();toggleMarkDone();}
    else if(e.key==='Escape'){exitCookingMode();}
    return;
  }
  if(e.key==='Escape'){
    if(document.getElementById('timerModal').classList.contains('active'))closeTimer();
    else if(document.getElementById('recipeModal').style.display==='flex')closeRecipe();
    else if(document.getElementById('fullImageModal').classList.contains('active'))closeFullImage();
  }
}
...
function selectPlate(category,plateElement){
  currentCategory=category;plateSelected=true;
  const plates=document.querySelectorAll('.plate');
  const wrapper=document.getElementById('plateWrapper');
  if(!wrapper) return;
  const wrapperWidth=wrapper.offsetWidth;
...
function spinWheel(){
  ...
  const plates=document.querySelectorAll('.plate');
  if(!plateSelected){
    if(allRecipes.length===0){showToast('Loading recipes... Please wait!','error');return;}
    plates.forEach((p)=>{
    ...
    setTimeout(()=>{
      plates.forEach(p=>{p.classList.remove('spinning','active');});
  ...
  if(currentRecipes.length===0){showToast('No recipes found for this category','error');return;}
  ...
  setTimeout(()=>{
    ...
    showRecipe(randomRecipe);
    btn.classList.remove('spinning');btn.style.pointerEvents='all';
    if(activePlate)activePlate.classList.remove('spinning');
  },3000);
}

window.addEventListener('resize',()=>{if(!plateSelected)setTimeout(()=>arrangePlatesInCircle(),150);});
const recipeModalEl = document.getElementById('recipeModal');
if (recipeModalEl) {
  recipeModalEl.addEventListener('click',(e)=>{if(e.target.id==='recipeModal')closeRecipe();});
}
...
function initInteractiveSplash() {
  ...
  enterBtn.innerHTML = '<span class="btn-icon">🏮</span><span>Enter Site</span>';
...
function handleLanternClick(event, lantern, index) {
  ...
  scroll.style.left = (lanternRect.left - splashRect.left + lanternRect.width/2 - 90) + 'px';
  scroll.style.top = (lanternRect.top - splashRect.top + lanternRect.height - 12) + 'px';
...
function initSplash() {
  ...
  .add({ targets: '.splash-seal', opacity: [0, 1], scale: [0, 1], duration: 600, easing: 'easeOutElastic(1, .6)' })
  .add({ targets: '.floating-lantern', opacity: [0, 0.7], translateY: [0, -30], duration: 800, delay: anime.stagger(200) }, '-=500')
  .add({ targets: '.splash-title-jp', opacity: [0, 1], translateZ: [-100, 0], rotateX: [20, 0], scale: [0.8, 1], duration: 700 }, '-=800')
  .add({ targets: '.splash-divider', opacity: [0, 1], scaleX: [0, 1], duration: 600 }, '-=600')
  .add({ targets: '.splash-title-en', opacity: [0, 1], translateY: [20, 0], duration: 600 }, '-=400')
  .add({ targets: '.splash-subtitle', opacity: [0, 1], translateY: [20, 0], duration: 500 }, '-=300')
  .add({ targets: '.splash-tagline', opacity: [0, 1], translateY: [10, 0], duration: 500 }, '-=200');
}
...
async function submitRecipe(event) {
    ...
    if (submitBtn) {
        ...
        submitBtn.innerHTML = '<span>⏳</span><span>Submitting to Community...</span>';
    }
    
    try {
        const cuisineSelect = document.getElementById('recipeCuisine');
        const customCuisineInput = document.getElementById('customCuisine');
        const finalCuisine = cuisineSelect.value === 'Other' 
        ...
        const recipeData = {
            ...
            chef_name: document.getElementById('chefName').value.trim() || 'Anonymous Chef',
...
function showCommunityPage() {
    ...
    anime({targets:'#community-page', opacity:[0,1], translateY:[30,0], duration:800, easing:'easeOutQuad'});
}
    ...
    recipeForm.addEventListener('submit', async (e) => {
        ...
        try {
            const recipeData = {
                ...
                servings: parseInt(document.getElementById('servings-input').value) || 1,
...
async function loadCommunityRecipes() {
    ...
    if (!grid) return;
    grid.innerHTML = '<div class="empty-state"><h3>Loading community recipes...</h3></div>';
    try {
        ...
        grid.innerHTML = recipes.map(recipe => {
            const cuisineObj = (typeof cuisines !== 'undefined' && Array.isArray(cuisines))
        ...
        grid.innerHTML = '<div class="empty-state"><h3>Error loading recipes</h3><p>Please try again later.</p></div>';
    }
}
...
function openCommunityRecipe(recipe) {
    ...
    const mappedRecipe = {
        ...
        servings: recipe.servings,
        chefTip: recipe.chef_tip ? `${recipe.chef_tip} — ${recipe.chef_name}` : `Shared by ${recipe.chef_name || 'Community Chef'}`,
        strInstructions: Array.isArray(recipe.instructions) ? recipe.instructions.join('\n') : recipe.instructions,
